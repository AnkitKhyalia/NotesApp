...
class RealtimeDbRespository @Inject constructor(
...
//                    val item = it.getValue(RealtimeModelResponse.RealtimeItems::class.java)
//                    item?.let {
//                        RealtimeModelResponse(it, key = it.key)
//                    }
//                }
    ...
    val valueEvent = object : ValueEventListener {
        ...
        override fun onDataChange(snapshot: DataSnapshot) {
            val items = snapshot.children.mapNotNull {
                ...
                val item = it.getValue(RealtimeModelResponse.RealtimeItems::class.java)
                item?.let {
                    RealtimeModelResponse(it, key =curr.key)
                }
            }
            trySend(ResultState.Success(items))
        }
